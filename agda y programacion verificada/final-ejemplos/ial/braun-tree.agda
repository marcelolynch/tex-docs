...
bt-remove-min (bt-node a bt-empty (bt-node _ _ _ _) (inj₂ ()))
bt-remove-min (bt-node a (bt-node{n'}{m'} a' l' r' u') bt-empty p) rewrite +0 (n' + m')= a , bt-node a' l' r' u'
...
bt-remove-min (bt-node a (bt-node a1 l1 r1 u1) (bt-node a2 l2 r2 u2) u) | a1' , l'
    with if a1' <A a then (a1' , a2) else (a2 , a1')
bt-remove-min (bt-node a (bt-node{n1}{m1} a1 l1 r1 u1) (bt-node{n2}{m2} _ l2 r2 u2) u) | _ , l' | smaller , other
    rewrite +suc (n1 + m1) (n2 + m2) | +comm (n1 + m1) (n2 + m2) = a , bt-node smaller (bt-node other l2 r2 u2) l' (lem u) where
    lem : ∀ {x y} → suc x ≡ y ∨ suc x ≡ suc y → y ≡ x ∨ y ≡ suc
    lem (inj₁ p) = inj₂ (sym p)
